...
import { getHeroTranslations, getResumeTranslations, getProjectsTranslations } from "@/app/actions/language";
import { Locale } from "@/lib/locales";
interface HeroSectionProps {
  locale: Locale
}
export default async function HeroSection({ locale }: HeroSectionProps) {
  const [tHero, tProjects, tResume] = await Promise.all([
    getHeroTranslations(locale),
    getProjectsTranslations(locale),
    getResumeTranslations(locale)
  ])
  return (
          ...
          <div className="space-y-8 max-w-4xl">
            ...
            >
              <div className="flex flex-wrap justify-center gap-4 pt-4">
                ...
                <span className="text-sm font-medium text-foreground">{tProjects('seguidoress')}</span>
                <Zap className="w-4 h-4 text-accent" />
                <span className="text-sm font-medium text-foreground">{tHero('title_frontend')}</span>
                <span className="text-sm font-medium text-foreground">{tProjects('spiritory')}</span>
              </div>
            </Motion>
